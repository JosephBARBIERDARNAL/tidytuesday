...
df = pd.read_csv(url)
df = (
    df.dropna(subset=["install_year", "country_name"])
    .assign(
        install_decade=(df["install_year"] // 10) * 10,
    )
    .query("1970 <= install_decade <= 2020")
)
df["nb_install"] = df.groupby(["country_name", "install_decade"])[
    "country_name"
].transform("size")
df = (
    pd.pivot_table(
        df, index="country_name", values="nb_install", columns="install_decade"
    )
    .dropna()
    .reset_index()
)
...
df = pd.melt(
    df, id_vars="country_name", value_name="nb_install", var_name="decade"
).pivot(index="decade", columns="country_name", values="nb_install")
df["decade"] = df.index
df.head()
...
plt.savefig("src/other/bumplot/output.png", dpi=300, bbox_inches="tight")
